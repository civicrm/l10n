 ...
 *   The string containing the filename the string's in.
 * @param string|int $input
 *   The string containing the msgid/msgstr block.
 * @param array $filelist
 *   The array containing the version info of the files.
 * @param bool $get
 *   The boolean switch whether the call is storing
 *   something or trying to get the whole storage back.
 *
 * @return array
 *   The array with the whole storage; only when $get == true.
 */
...
function writeFiles() {
  ...
  foreach ($output as $file => $content) {
    ...
    //$fp = fopen($file, 'w');
    fwrite(STDOUT, str_replace("--VERSIONS--", $filelist, implode("", $content)));
    //fclose($fp);
