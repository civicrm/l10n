...
- Run this script (where 6.0 is the latest stable version of CiviCRM and 5.75 is the current ESR):
    $ ./bin/build-unified-pots.sh ~/buildkit/build/smaster/web/core po/pot '6.0 5.75' 2>&1 | tee pots.log
...
for rel in $releases; do
  ...
  // @todo Create 2 build/sites and then merge merge the pot files
  civibuild create l10n-{$rel} --type standalone-clean --civi-ver $rel
